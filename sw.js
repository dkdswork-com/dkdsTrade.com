const CACHE='trade-memo-private-v3-1';
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['./','./index.html','./guide.html','./manifest.json']))));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!=='trade-memo-private-v3-1').map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
